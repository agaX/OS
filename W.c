      ...
      case operator:
        ...
        if (first->next->c != '#' && first->next->c != '(') {
          ...
          if (x1 < x2) {
            elem->next = first->next;
            first->next = elem;
            // sciagam ze sotsu to co bylo i wrzucam do wyjscia, a - dokladam na
            // stos
          } else {
            pom = first->next;
            elem->next = pom->next;
    ...
    if (guard->c == '&') {
        ...
        switch (fork()) {
          ...
          default:
            ...
            guard = act;
            while (act != NULL) {
                if (write(pipe_dsc[0][1], &act->c, sizeof(char)) == -1) syserr("W(), write(pipe_dsc[0][1])");
                act = act->next;
                free(guard);
                guard = act;
            }
            c = ' ';
            while (c != '#') {
                if (read(pipe_dsc[1][0], &c, sizeof(char)) == -1 ) syserr("W(), read(pipe_dsc[1][0])");
